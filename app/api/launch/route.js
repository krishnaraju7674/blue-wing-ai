...
// Helper to launch UWP/Store apps by searching for them dynamically
async function launchUWPApp(keyword) {
  const scriptPath = join(tmpdir(), `bw_launch_${Date.now()}.ps1`);
  const script = `
$app = Get-AppxPackage *${keyword}* | Select-Object -First 1
if ($app) {
  $id = $app.PackageFamilyName
  Start-Process "shell:AppsFolder\\$id!App"
  Write-Output "LAUNCHED:$id"
} else {
...
const APPS = {
  ...
  'spotify': { uwp: 'Spotify', close: 'taskkill /IM Spotify.exe /F', name: 'Spotify', webUrl: 'https://open.spotify.com' },
  ...
  // Communication — UWP apps use 'uwp' keyword for dynamic detection
  'discord': { uwp: 'Discord', close: 'taskkill /IM Discord.exe /F', name: 'Discord', webUrl: 'https://discord.com/app' },
  'slack': { uwp: 'Slack', close: 'taskkill /IM slack.exe /F', name: 'Slack', webUrl: 'https://app.slack.com' },
  'teams': { uwp: 'MSTeams', close: 'taskkill /IM ms-teams.exe /F', name: 'Teams', webUrl: 'https://teams.microsoft.com' },
  'microsoft teams': { uwp: 'MSTeams', close: 'taskkill /IM ms-teams.exe /F', name: 'Teams', webUrl: 'https://teams.microsoft.com' },
  'whatsapp': { uwp: 'WhatsApp', close: 'taskkill /IM WhatsApp.exe /F', name: 'WhatsApp', webUrl: 'https://web.whatsapp.com' },
  'telegram': { uwp: 'Telegram', close: 'taskkill /IM Telegram.exe /F', name: 'Telegram', webUrl: 'https://web.telegram.org' },
  'zoom': { uwp: 'Zoom', close: 'taskkill /IM Zoom.exe /F', name: 'Zoom', webUrl: 'https://zoom.us' },
  'skype': { uwp: 'Skype', close: 'taskkill /IM Skype.exe /F', name: 'Skype', webUrl: 'https://web.skype.com' },
  ...
  // Gaming
  'steam': { open: 'start "" "C:\\Program Files (x86)\\Steam\\steam.exe"', close: 'taskkill /IM steam.exe /F', name: 'Steam', webUrl: 'https://store.steampowered.com' },
  'epic games': { open: 'start "" "C:\\Program Files (x86)\\Epic Games\\Launcher\\Portal\\Binaries\\Win64\\EpicGamesLauncher.exe"', close: 'taskkill /IM EpicGamesLauncher.exe /F', name: 'Epic Games', webUrl: 'https://store.epicgames.com' },
  'epic': { open: 'start "" "C:\\Program Files (x86)\\Epic Games\\Launcher\\Portal\\Binaries\\Win64\\EpicGamesLauncher.exe"', close: 'taskkill /IM EpicGamesLauncher.exe /F', name: 'Epic Games', webUrl: 'https://store.epicgames.com' },
  'minecraft': { uwp: 'Minecraft', close: 'taskkill /IM Minecraft.exe /F', name: 'Minecraft', webUrl: 'https://www.minecraft.net' },
  'roblox': { uwp: 'Roblox', close: 'taskkill /IM RobloxPlayerBeta.exe /F', name: 'Roblox', webUrl: 'https://www.roblox.com' },
...
export async function POST(req) {
  try {
    ...
    if (app) {
      // Handle close
      if (isClose) {
        const cmd = app.close || `taskkill /IM ${t}.exe /F`;
        try { await execAsync(cmd); } catch { /* app may not be running */ }
        return NextResponse.json({ text: `${app.name} terminated.`, hash: hash(), success: true });
      }

      // Handle UWP apps (WhatsApp, Telegram, Discord, Spotify, etc.)
      if (app.uwp) {
        try {
          const result = await launchUWPApp(app.uwp);
          if (result.startsWith('LAUNCHED')) {
            return NextResponse.json({ text: `Launching ${app.name}, Sir.`, hash: hash(), success: true });
          }
          // App not installed — fall back to web
          if (app.webUrl) {
            return NextResponse.json({ text: `${app.name} not installed. Opening web version, Sir.`, hash: hash(), success: true, type: 'web', url: app.webUrl });
          }
          return NextResponse.json({ text: `${app.name} is not installed on this system, Sir.`, hash: hash(), success: false });
        } catch (err) {
          // UWP launch failed — try web fallback
          if (app.webUrl) {
            return NextResponse.json({ text: `Opening ${app.name} web version, Sir.`, hash: hash(), success: true, type: 'web', url: app.webUrl });
          }
          return NextResponse.json({ text: `${app.name} launch failed.`, hash: hash(), success: false });
        }
      }

      // Handle standard apps
      try {
        await execAsync(app.open, { timeout: 10000 });
        return NextResponse.json({ text: `Launching ${app.name}, Sir.`, hash: hash(), success: true });
      } catch (launchErr) {
        const webUrl = app.webUrl || WEB_APPS[t];
        if (webUrl) {
          return NextResponse.json({ text: `Native ${app.name} not found. Opening web version, Sir.`, hash: hash(), success: true, type: 'web', url: webUrl });
        }
        return NextResponse.json({ text: `${app.name} launch failed: ${launchErr.message?.substring(0, 80)}`, hash: hash(), success: false });
      }
    }
    ...
    if (!isClose) {
      try {
        await execAsync(`start ${t}`, { timeout: 8000 });
        return NextResponse.json({ text: `Manual override: Initializing ${t}, Sir.`, hash: hash(), success: true });
      } catch {
        // Last resort: check if there's a matching web app
        const webMatch = Object.keys(WEB_APPS).find(key => key.includes(t) || t.includes(key));
        if (webMatch) {
          return NextResponse.json({ text: `Opening ${webMatch} in browser, Sir.`, hash: hash(), success: true, type: 'web', url: WEB_APPS[webMatch] });
        }
