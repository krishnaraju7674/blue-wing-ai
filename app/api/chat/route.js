...
function getLocalResponse(cmd) {
  ...
  // Flexible time/date matching — catches all common variants
  const timeWords = ['time', 'clock', 'hour'];
  const dateWords = ['date', 'day', 'today'];
  const askWords = ['what', 'whats', "what's", 'tell', 'show', 'current', 'the'];
  
  if (timeWords.some(w => cmd.includes(w)) && (cmd.length < 40) && 
      (askWords.some(w => cmd.includes(w)) || cmd === 'time' || cmd === 'clock')) {
    const now = new Date();
    const time = now.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
    return `It's ${time}, Sir.`;
  }
  if (dateWords.some(w => cmd.includes(w)) && (cmd.length < 40) && 
      (askWords.some(w => cmd.includes(w)) || cmd === 'date' || cmd === 'today')) {
    const now = new Date();
    const date = now.toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    return `Today is ${date}, Sir.`;
  }
  // Combined time+date
  if ((cmd.includes('time') && cmd.includes('date')) || cmd.includes('datetime') || cmd.includes('time and date')) {
    const now = new Date();
    const time = now.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true });
    const date = now.toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    return `It's ${time} on ${date}, Sir.`;
  }
